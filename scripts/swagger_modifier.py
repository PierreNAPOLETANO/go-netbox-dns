...
# Adapted from go-netbox https://github.com/netbox-community/go-netbox/blob/18aea9a4ac2c7a995e1ed15baa105f9032b4239e/scripts/swagger_modifier.py
...
def cli_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description=("Modifications on swagger.json."))
    parser.add_argument(
        "-v", "--verbose", action="count", default=0, help="Increase output verbosity."
    )
    parser.add_argument("-o", "--output", default="swagger.json", help="Output file")
    parser.add_argument(
        "input",
        nargs="?",
        default="swagger_original.json",
...
def complete_data(input_file, log):
    ...
    # Filter netbox-dns paths
    paths = {}
    for path in data["paths"]:
        if path.startswith("/plugins/netbox-dns"):
            paths[path] = data["paths"][path]
    data["paths"] = paths

    # Fitler netbox-dns properties
    definitions = {}
    for definition in NETBOX_DNS_DEFINITIONS:
        definitions[definition] = data["definitions"][definition]
        ...
        for verb in data["paths"][path]:
            if verb in verbs:
                if "default" not in data["paths"][path][verb]["responses"]:
                    log.info("Adding default in {} verb of {} path".format(verb, path))
                    data["paths"][path][verb]["responses"].update(missing_default)
                else:
                ...
                if def_property in prop:
                    for k in modify_properties[prop]:
                        if k not in def_properties[def_property]:
                            log.info("Adding {} in {} property".format(k, prop))
                            def_properties[def_property].update(modify_properties[prop])
